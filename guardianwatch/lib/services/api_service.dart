...
class ApiService {
  ...
  Future<void> saveInsights(List<Insight> insights) async {
    ...
    if (_base == 'https://guardianwrist-api-xxxxxxxx-uc.a.run.app') {
      print('Backend not configured, insights saved locally only');
      return;
    }

    try {
      final res = await http.post(
        Uri.parse('$_base/insights/save'),
        headers: await _headers(),
        body: jsonEncode({
      ...
      if (res.statusCode != 201 && res.statusCode != 200) {
        print('Failed to save insights: ${res.statusCode}');
      }
    } catch (e) {
