...
class BleProvider extends ChangeNotifier {
  ...
  /// Request runtime permissions required for BLE scanning and connection
  Future<bool> requestPermissions() async {
    if (Platform.isAndroid) {
      ...
      // On Android 12+ scan/connect are required. On older versions, location is required.
  ...
  Future<void> startScan() async {
    ...
    if (!hasPermission) {
      _setError('Bluetooth and Location permissions are required to scan.');
      return;
    }
  ...
  Future<void> connectTo(BluetoothDevice device) async {
    final hasPermission = await requestPermissions();
    if (!hasPermission) {
      _setError('Bluetooth permissions are required to connect.');
      return;
    }
  ...
  void _listenToData() {
    _dataSub = _ble.dataStream.listen((incoming) async {
      ...
      if (incoming.heartRate != null) {
        _insightProvider.feedHeartRate(incoming.heartRate!);
      }
      if (incoming.spo2 != null) {
        _insightProvider.feedSpO2(incoming.spo2!);
      }
      if (incoming.temperature != null) {
        _insightProvider.feedTemperature(incoming.temperature!);
      }
      if (incoming.ecgMv != null && incoming.ecgMv!.isNotEmpty) {
        _insightProvider.feedEcgData(incoming.ecgMv!);
      }
  ...
  void startMockData() {
    ...
    _mockDataTimer = Timer.periodic(const Duration(milliseconds: 100), (timer) {
      ...
      final mockHr = 60 + (second % 41);
      final mockSpo2 = 95 + (second % 5);
      final mockTemp = 36.2 + (second % 8) / 10.0;

      final mockEcg = <double>[];
      for (int i = 0; i < 50; i++) {
        mockEcg.add(0.5 * sin(i / 10 * 3.14159));
      }
